...
func TestUpgradePathFromPreviousRelease(t *testing.T) {
	...
	gooseRun(t, db, "up")
	if v := queryVersion(t, db); v != 6 {
		t.Fatalf("latest version = %d, want 6", v)
	}
...
func TestMigrationIdempotentRerun(t *testing.T) {
	...
	if v := queryVersion(t, db); v != 6 {
		t.Fatalf("version after rerun = %d, want 6", v)
	}
...
func TestConcurrentRunnersSingleLock(t *testing.T) {
	...
	if v := queryVersion(t, db); v != 6 {
		t.Fatalf("version after concurrent migrators = %d, want 6", v)
	}
...
func TestInterruptedMigrationRecoversSafely(t *testing.T) {
	...
	gooseRun(t, db, "up")
	if v := queryVersion(t, db); v != 6 {
		t.Fatalf("version after retry = %d, want 6", v)
	}
	if err := conn.QueryRow(ctx, `
		SELECT EXISTS (SELECT 1 FROM information_schema.columns
		WHERE table_schema='saoaf' AND table_name='outbox_event' AND column_name='published_seq')`).Scan(&colExists); err != nil {
...
`), 0o644); err != nil {
	...
	if v := queryVersion(t, db); v != 6 {
		t.Fatalf("version after failed migration = %d, want 6", v)
	}
	var col bool
...
func TestMigrationUpDownUpRoundTrip(t *testing.T) {
	...
	// up → down → up
	gooseRun(t, db, "up")
	// goose down rolls back the most recent migration (v6 → v5), exercising
	// 00006's Down section (plan tables + triggers + index drops)
	if out, err := tryGooseRun(db, "down"); err != nil {
		t.Fatalf("goose down failed (Down section broken): %v\n%s", err, out)
	}
	if v := queryVersion(t, db); v != 5 {
		t.Fatalf("version after down = %d, want 5", v)
	}
	gooseRun(t, db, "up")
	if v := queryVersion(t, db); v != 6 {
		t.Fatalf("version after up-down-up = %d, want 6", v)
	}
}
