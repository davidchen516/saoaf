...
| P3-2 Count disclosure errors (negative ×6 actually 5; 22 assertions actually 21) | Two-round story: the R1-round prose correction silently no-op'd (the rework table claimed done while the body stayed stale — the same failure shape the I17 R2 review blocked on); after R2 blocked on it, this round rewrote the three body lines assert-guarded with grep verification (now ×8 / 21 assertions / gate negatives=19) | Text + git log 0b96fb5 |
